...
class DinnerPlates:
    ...
    def push(self, val: int) -> None:
        if self.mnh:  # the left most not-full stack
            ...
            # update the candidate of right most non-emapty
            heappush(self.mxh, -idx)

    def pop(self) -> int:
        print(" dq len: ", len(self.dq))
        pp(self.dq)
        print(" mnh: ", self.mnh)
        print(" mxh: ", self.mxh)
        if len(self.dq) == 0:
    ...
    def popAtStack(self, index: int) -> int:
        if index >= len(self.dq):
            return -1  # index exceeding max current stacks

        stk = self.dq[index]  # the current stack
        if not stk:
            return -1  # if this guy is empty, return -1
        ...
        if len(stk) == 0:
            try:
                print(" removing -index: ", index, " the mxh: ", self.mxh)

                idx = self.mxh.index(-index)
                print(" found index: ", idx)
                del self.mxh[idx]
                print(" after remove: ", self.mxh)
                heapify(self.mxh)
            except Exception as e:
