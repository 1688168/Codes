class Solution:
    def maximumBeauty(self, flowers: List[int], newFlowers: int, target: int, full: int, partial: int) -> int:
        ...
        if len(flowers)==0: return ret0 #all gardens are complete
        ...
        for ii in reversed(range(N)):
            ...
            if presum[ii]+newFlowers >= (ii+1)*(target-1): #per above, we cannot have idx <= ii greater or equal to target
                ret= max(ret, (target-1)*partial+ (N-1-ii)*full) # from ii+1~N-1 inclusive is N-1 - (ii-1)+1=N-ii-1+1+1=N-1-ii
            ...
            newFlowers -= (target-flowers[ii]) #by moving ii to the left, indicating we fill flowers[ii] to target. and rest for 0~(ii-1)

        if newFlowers >=0: # we can fill all > target
            return max(ret+ret0,N*full)
        return ret+ret0