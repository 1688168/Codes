    ...
    def shortestPath(self, grid: List[List[int]], k: int) -> int:
        ...
        while (sz:=len(dq)) > 0: #take sz for each level
            for _ in range(sz):# for each level
                xx, yy, zz = dq.popleft() #all nodes on same level, level starts from 0. zz is the initial state when etering a new grid
                if xx < 0 or xx >=M or yy < 0 or yy >= N or (xx, yy, zz) in visited: continue
                visited.add((xx, yy, zz)) # recording the zz-1 as the initial state on this grid is visited
             
                if grid[xx][yy]==1:#if obstacle, consume one k
                    if zz >= k: continue #if the initial kk state is already k, we cannot proceed from here
                    zz+=1
