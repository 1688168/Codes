class Solution:
    def profitableSchemes(self, n: int, minProfit: int, group: List[int], profit: List[int]) -> int:
        ...
        * -> num of ways to achive minProfit
        ...
        """
        dp = [[[0]*(minProfit+1) for _ in range(n+1)] for _ in range(N+1)]
        dp[0][0][0]=1

        # so we need to start from zero cuz we are inferring ii+1 (the opposite direction)
        # up to (N-1) project
        for ii in range(N): #for each project, we DONOT need to cover Nth proj, as we do not need to infer anything beyound last project
            for jj in range(n+1): # up tp n ppl
                for kk in range(minProfit+1):# up to minProfit
