...
class LFUCache:
    ...
    def get(self, key: int) -> int:
        if self.capacity <= 0: return -1 # boundary check on capacity

        f=self.k2f.get(key, -1)  # if key is not found, value is -1
        if f==-1: return -1

        kv=self.f2kv.get(f, None)  # get kv map per frequency. should not be None

        if kv is None: return -1
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if len(self.k2f) >= self.capacity:
            ...
            key_to_remove, value_to_remove=self.f2kv[self.min_freq].popitem(last=False) # understand popitem
